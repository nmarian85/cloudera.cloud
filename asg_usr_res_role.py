...
def dump_assign_group_resource_role_json(cdp_env_crn, cdp_role, user, json_skel):
...
def dump_unassign_group_resource_role_json(cdp_env_crn, cdp_role, user, json_skel):
    resource_role_crn = f"{requests_ops.DEFAULT_IAM_CRN}:resourceRole:{cdp_role}"
    assign_group_role_json = dict(json_skel)
    assign_group_role_json["resourceRoleCrn"] = resource_role_crn
    assign_group_role_json["resourceCrn"] = cdp_env_crn
    ...
    for group, roles in cdp_env_info["cdp_igam_groups"].items():
        for role in roles:
            ...
            if not dryrun:
                ...
                click.echo(
                    f"Action {action} on cdp user {group} on env {cdp_env_name} assigning role {role} DONE"
                )
