...
EOF
        ...
        DL)
            OUT=$(cdp datalake describe-datalake --datalake-name $DLAKE_NAME || true)
            [ -z "$OUT" ] || STATUS=$(echo "$OUT" | python -c 'import json,sys; print(json.load(sys.stdin)["datalake"]["status"])')
        ;;
    esac
